...
for ref_sample in all_samples.copy():
    try:
            ...
            for row in cur:
                ...
                print("", file=sys.stderr)
                print("ATTENTION! Sample " + existing_sample + " is already present in the " + genome + " database. IT WILL BE SKIPPED!", file=sys.stderr)
                ...
                print("  - Before re-uploading the data you need remove this sample from the " + genome + " database using the following command: < WRITE FINAL COMMAND > ", file=sys.stderr)
    except mariadb.Error as e:
        print(f"Error: {e}", file=sys.stderr)
        sys.exit(1)
